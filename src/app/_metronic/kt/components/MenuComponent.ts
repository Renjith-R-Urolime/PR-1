...
class MenuComponent {
  ...
  private _hideAccordion = (item: HTMLElement) => {
    ...
    if (sub) {
      slideUp(sub, this.options.accordion.slideSpeed, () => {
        ...
        const link = this._getItemLinkElement(item)
        if(link) {
          link.classList.remove('active', 'rounded-bottom-0');
        }
  ...
  private _toggle = (item: HTMLElement) => {
    ...
    if (this._isItemSubShown(item) === true) {
      ...
      const link = this._getItemLinkElement(item)
      if(link) {
        link.classList.add('active', 'rounded-bottom-0');
      }
      this._show(item)
    }
  }
  ...
  private _link = (element: HTMLElement, e: Event) => {
    ...
    MenuComponent.hideAccordions(element)
  ...
  public static hideAccordions = (skip: HTMLElement | undefined) => {
    const items = document.querySelectorAll<HTMLElement>(
      '.show.menu-accordion[data-kt-menu-trigger]'
    )
    ...
    if (items && items.length > 0) {
      for (let i = 0, len = items.length; i < len; i++) {
        ...
        if (menu && menu.getItemSubType(item) === 'accordion') {
          if (skip) {
            if (
              // @ts-ignore
              menu.getItemSubElement(item).contains(skip) === false &&
