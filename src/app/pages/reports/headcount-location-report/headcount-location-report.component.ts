...
export class HeadcountLocationReportComponent {
  ...
  prepareGrid() {
    ...
    columnDefinitions.push({ id: 'location', name: 'Location', field: 'location', headerCssClass: 'red-header' });
  ...
  getSubsideryHeadCount(filterCondition) {
    ...
    this.apiService.get(`reports/location/head-count?limit=max&${queryString}`).subscribe({
      next: (res: any) => {
        if (res && res.data) {
  ...
  createGroupedHeaders() {
    ...
    if (preHeaderPanel) {
      ...
      locationHeaderDiv.className = 'header-group common-report-header';
      locationHeaderDiv.textContent = 'Location';
      preHeaderPanel.appendChild(locationHeaderDiv);
  ...
  processApiData(apiData: any[]) {
    ...
    apiData.forEach(monthData => {
      ...
      monthData.entries.forEach((location: any) => {
        let existingEntry = mockDataset.find(entry => entry.name === location.name);
        if (!existingEntry) {
          existingEntry = {
            id: id++,
        ...
        existingEntry[`${formattedMonthYear}Count`] = location.count;
        existingEntry[`${formattedMonthYear}CountPercent`] = location.percentage;
      });
    });
  ...
  updateFooter() {
    if (this.angularGrid && this.angularGrid.slickGrid && this.angularGrid.gridService) {
      ...
      const footerRow = grid.getFooterRowColumn('location');
      footerRow.innerHTML = '';

      const totalItemElement = document.createElement('div');
      totalItemElement.textContent = `Total Items: ${this.totalRow.location}`;
  ...
  downloadReportCSV(): void {
    ...
    this.downloadCSVFromData(this.dataset, 'location-headcount-report.csv');
  }
  ...
  downloadReportExcel(): void {
    ...
    // this.downloadExcelFromData(this.dataset, 'location-headcount-report.xlsx');
  }
