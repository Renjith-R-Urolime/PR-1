...
export class HeadcountNationalityReportComponent {
  ...
  prepareGrid() {
    ...
    columnDefinitions.push({ id: 'nationality', name: 'Nationality', field: 'nationality', headerCssClass: 'red-header' });
  ...
  getSubsideryHeadCount(filterCondition) {
    ...
    this.apiService.get(`reports/nationality/head-count?limit=max&${queryString}`).subscribe({
      next: (res: any) => {
        if (res && res.data) {
  ...
  createGroupedHeaders() {
    ...
    if (preHeaderPanel) {
      ...
      const nationalityHeaderDiv = document.createElement('div');
      nationalityHeaderDiv.className = 'header-group common-report-header';
      nationalityHeaderDiv.textContent = 'Nationality';
      preHeaderPanel.appendChild(nationalityHeaderDiv);
  ...
  processApiData(apiData: any[]) {
    ...
    apiData.forEach(monthData => {
      ...
      monthData.entries.forEach((nationality: any) => {
        let existingEntry = mockDataset.find(entry => entry.name === nationality.name);
        if (!existingEntry) {
          existingEntry = {
            id: id++,
        ...
        existingEntry[`${formattedMonthYear}Count`] = nationality.count;
        existingEntry[`${formattedMonthYear}CountPercent`] = nationality.percentage;
      });
    });

    const sortedEntries = mockDataset.sort((a, b) => {
      if (a.nationality === 'No Nationality') return 1;
      if (b.nationality === 'No Nationality') return -1;
      return 0;
    });
  ...
  updateFooter() {
    if (this.angularGrid && this.angularGrid.slickGrid && this.angularGrid.gridService) {
      ...
      const footerRow = grid.getFooterRowColumn('nationality');
      footerRow.innerHTML = '';

      const totalItemElement = document.createElement('div');
      totalItemElement.textContent = `Total Items: ${this.totalRow.nationality}`;
  ...
  downloadReportCSV(): void {
    ...
    this.downloadCSVFromData(this.dataset, 'nationality-headcount-report.csv');
  }
  ...
  downloadReportExcel(): void {
    ...
    // this.downloadExcelFromData(this.dataset, 'nationality-headcount-report.xlsx');
  }
