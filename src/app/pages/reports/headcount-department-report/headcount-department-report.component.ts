...
export class HeadcountDepartmentReportComponent {
  ...
  prepareGrid() {
    ...
    columnDefinitions.push({ id: 'department', name: 'Department', field: 'department', headerCssClass: 'red-header' });
  ...
  getSubsideryHeadCount(filterCondition) {
    ...
    this.apiService.get(`reports/department/head-count?limit=max&${queryString}`).subscribe({
      next: (res: any) => {
        if (res && res.data) {
  ...
  createGroupedHeaders() {
    ...
    if (preHeaderPanel) {
      ...
      departmentHeaderDiv.className = 'header-group common-report-header';
      departmentHeaderDiv.textContent = 'Department';
      preHeaderPanel.appendChild(departmentHeaderDiv);
  ...
  processApiData(apiData: any[]) {
    ...
    apiData.forEach(monthData => {
      ...
      monthData.entries.forEach((department: any) => {
        let existingEntry = mockDataset.find(entry => entry.name === department.name);
        if (!existingEntry) {
          existingEntry = {
            id: id++,
        ...
        existingEntry[`${formattedMonthYear}Count`] = department.count;
        existingEntry[`${formattedMonthYear}CountPercent`] = department.percentage;
      });
    });

    const sortedEntries = mockDataset.sort((a, b) => {
      if (a.department === 'No Department') return 1;
      if (b.department === 'No Department') return -1;
      return 0;
    });
  ...
  updateFooter() {
    if (this.angularGrid && this.angularGrid.slickGrid && this.angularGrid.gridService) {
      ...
      const footerRow = grid.getFooterRowColumn('department');
      footerRow.innerHTML = '';

      const totalItemElement = document.createElement('div');
      totalItemElement.textContent = `Total Items: ${this.totalRow.department}`;
  ...
  downloadReportCSV(): void {
    ...
    this.downloadCSVFromData(this.dataset, 'department-headcount-report.csv');
  }
  ...
  downloadReportExcel(): void {
    ...
    // this.downloadExcelFromData(this.dataset, 'department-headcount-report.xlsx');
  }
