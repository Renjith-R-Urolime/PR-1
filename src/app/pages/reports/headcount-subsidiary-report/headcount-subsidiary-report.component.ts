...
import { Aggregators, AngularGridInstance, Column, ExtensionName, Formatters, GridOption, GroupTotalFormatters, Grouping, GroupingGetterFunction } from 'angular-slickgrid';
...
export class HeadcountSubsidiaryReportComponent {
  ...
  getSubsideryHeadCount(filterCondition) {
    ...
    this.apiService.get(`reports/subsidiary/head-count?limit=max&${queryString}`).subscribe({
      next: (res: any) => {
        if (res && res.data) {
  ...
  createGroupedHeaders() {
    ...
    if (preHeaderPanel) {
      ...
      const subsidiaryHeaderDiv = document.createElement('div');
      subsidiaryHeaderDiv.className = 'header-group common-report-header';
      subsidiaryHeaderDiv.textContent = 'Subsidiary';
      preHeaderPanel.appendChild(subsidiaryHeaderDiv);
  ...
  processApiData(apiData: any[]) {
    ...
    apiData.forEach(monthData => {
      ...
      monthData.entries.forEach((subsidiary: any) => {
        let existingEntry = mockDataset.find(entry => entry.subsidiary === subsidiary.name);
        if (!existingEntry) {
          existingEntry = {
            id: id++,
